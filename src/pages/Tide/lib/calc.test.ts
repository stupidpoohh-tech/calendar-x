...
describe('limitOn', () => {
  ...
  it('잔고를 적은 날 이전에 지나간 발생분은 한도를 바꾸지 않는다', () => {
    // 잔고는 3/7 의 사실이다. 3/5 관리비는 그 숫자에 이미 들어 있다.
    const withPast = state({
      ...s,
      entries: [...s.entries, once('p', '관리비', 78_000, '2026-03-05')],
    });
  ...
  it('오늘 날짜로 적은 것은 한도에 잡힌다', () => {
    // 적어 둔 항목은 아직 일어나지 않은 계획이다. 오늘 것도 앞에 있다.
    const withToday = state({ ...s, entries: [...s.entries, once('t', '점심', 9_000, '2026-03-07')] });
    expect(limitOn(withToday, '2026-03-07', '2026-03-07'))
      .toBe(limitOn(s, '2026-03-07', '2026-03-07') - 9_000);
    expect(headlineLimit(withToday, '2026-03-07')).toBe(headlineLimit(s, '2026-03-07') - 9_000);
  });
  ...
  it('과거에 시작했어도 아직 안 끝난 기간은 남은 몫이 한도에 잡힌다', () => {
    ...
    // 잔고를 3/7 에 적었다 → 3/7~3/20(14일) 몫이 예약된다. 오늘 몫도 아직 앞에 있다.
    expect(limitOn(span('2026-03-01', '2026-03-20'), '2026-03-10', '2026-03-07')).toBe(930_000);
  ...
  it('예정 입금은 한도를 늘리지만, 다음 급여 자체는 이번 주기에 안 들어간다', () => {
    ...
    // 3/10 이 되면 끝점은 3/25 급여 전날이고, 용돈은 잔고(3/7)에 없으므로 더해진다.
    expect(headlineLimit(s3, '2026-03-10')).toBe(150_000);
  });
...
describe('limitOn — 기간 예산은 전액 예약으로 보인다', () => {
  ...
  it('잔고 기준일 다음 몫이 예약된다', () => {
    // 잔고를 3/4 에 적었고 기간은 3/5 에 시작한다 → 총액이 통째로 예약된다.
    const mid = limitOn(s, '2026-03-12', '2026-03-10');
    expect(mid).toBe(500_000 - 100_000);
...
describe('settle', () => {
  ...
  it('생활비 기간 중간에 정산하면 일할 페이스가 기준이 된다', () => {
    ...
    // 어제(3/9)까지 5일치 = 9,090 × 5 = 45,450원이 예정 페이스.
    // 오늘 몫은 아직 앞에 있어 흡수하지 않는다 — 한도가 그것을 센다.
    const result = settle(s, 254_550, new Date('2026-03-10T09:00:00+09:00'));
    expect(result.passedOut).toBe(45_450);
    expect(result.expected).toBe(254_550);
    expect(result.diff).toBe(0);
    // 정산 후에도 남은 기간 몫만 한도에서 빠진다 — 이중 차감이 없다.
    const after = state({
      ...s,
      balance: { amount: 254_550, checkedAt: '2026-03-10T09:00:00+09:00' },
    });
    expect(limitOn(after, '2026-03-15', '2026-03-10')).toBe(254_550 - (100_000 - 45_450));
...
describe('구간 규칙의 일관성', () => {
  ...
  it('정산 후 다시 계산해도 한도가 어긋나지 않는다', () => {
    ...
    // (3/7, 3/11] 사이 지나간 것은 3/10 월세뿐이다. 3/12 적금은 오늘 것이라
    // 아직 나가지 않은 계획으로 보고 흡수하지 않는다 — 한도가 그것을 센다.
    const result = settle(s, 1_000_000 - 600_000, new Date('2026-03-12T09:00:00+09:00'));
    expect(result.diff).toBe(0);
    const after = state({
      ...s,
