 ...
 *
 * 앞선 주석은 "클라이언트 SDK 에는 컬렉션을 가로지르는 트랜잭션이 없다" 고 적었다.
 * **사실이 아니다.** `runTransaction` 은 컬렉션을 가로질러 읽고 쓸 수 있고, 병합의
 * `createManyIfAbsent` 가 실제로 그렇게 쓰고 있다. 교체를 못 하는 진짜 이유는 셋이다.
 *
 *   1. **크기 상한.** 트랜잭션 한 건은 문서 500개까지다. 백업은 그보다 크다.
 *   2. **여러 번 나뉜다.** 500개를 넘으면 배치·트랜잭션 여러 건으로 쪼개야 하는데,
 *      그 사이는 원자적이지 않다. 중간에 끊기면 반쯤 덮인 상태로 남는다.
 *   3. **되돌릴 설계가 없다.** 덮이기 전 원본을 어디에도 굳혀 두지 않는다. 메모리에
 *      들고 있는 것은 안전장치가 못 된다 — 탭이 닫히면 그 원본도 사라진다.
 *
 * 그래서 지금 구조에서는 안전한 교체를 보장할 수 없다.
 *
...
export function planMerge(current: BackupData, incoming: BackupData): BackupData {
  ...
  return {
    ...
    /*
      ...
      지금 것을 남긴다는 규칙만 여기 적어 두고, 실제로 적용되지 않는다는 사실은
      화면이 `MERGE_SKIPS_RECOVERY` 로 사용자에게 알린다.
    */
...
/**
 * 병합이 회복 설정을 적용하지 않는다는 사실.
 *
 * 파일에 회복 설정이 들어 있는데 아무 말 없이 넘기면, 사용자는 간격·빚·기본 메모까지
 * 돌아온 줄 알고 설정을 다시 보지 않는다. 그러다 다음 회차가 엉뚱한 날에 뜬다.
 */
export const MERGE_SKIPS_RECOVERY =
  '파일에 회복 설정이 들어 있지만 “더하기”로는 적용되지 않습니다. '
 ...
 */
/*
  대안으로 "지울 항목을 직접 지운 뒤 더하라" 고 적었었다. 그 안내는 거둔다 —
  손으로 지우는 일이야말로 되돌릴 수 없고, 무엇을 지워야 파일과 같아지는지 사용자가
  알 방법도 없다. 교체가 위험해서 막아 놓고 더 위험한 수동 작업을 권할 수는 없다.
*/
export const REPLACE_DISABLED_REASON =
  ...
  + '그때까지는 “기존 데이터에 더하기”를 쓰세요 — 파일에만 있는 것이 더해지고 '
  + '지금 있는 것은 하나도 바뀌지 않습니다. 처음부터 다시 시작하려면 '
  + '새 계정을 만들어 그 계정으로 가져오는 편이 안전합니다.';
