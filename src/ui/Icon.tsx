...
function base({ size = 16, stroke = 'currentColor', ...rest }: Props) {
  return {
    width: size,
    height: size,
    viewBox: '0 0 24 24',
...
export const Icon = {
  ...
  Chevron: ({ dir = 'right', ...p }: Props & { dir?: 'left' | 'right' | 'up' | 'down' }) => {
    const points = {
    ...
    return <svg {...base(p)}><polyline points={points} /></svg>;
  },
  Filter: (p: Props) => (
  ...
  Star: ({ filled, fillColor = 'currentColor', ...p }: Props) => (
    <svg {...base(p)} fill={filled ? fillColor : 'none'}>
      <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
    </svg>
  ),
  Flame: ({ filled, fillColor = 'currentColor', ...p }: Props) => (
    <svg {...base(p)} fill={filled ? fillColor : 'none'}>
  ...
  Wallet: (p: Props) => (
    <svg {...base(p)}>
      ...
      <path d="M4 6v12a2 2 0 0 0 2 2h14v-4" /><path d="M18 12a2 2 0 0 0 0 4h4v-4z" />
    </svg>
  ),
  Bulb: (p: Props) => (
    <svg {...base(p)}>
  ...
  Pin: (p: Props) => (
    <svg {...base(p)}>
      ...
      <path d="M5 17h14l-1.6-2.4A2 2 0 0 1 17 13.5V4H7v9.5a2 2 0 0 1-.4 1.1L5 17z" />
    </svg>
  ),
  /** 감추기 / 다시 보이기. */
  EyeOff: (p: Props) => (
  ...
  Undo: (p: Props) => (
    <svg {...base(p)}>
      ...
      <path d="M3.5 13a9 9 0 1 0 2.6-7.1L3 9" />
    </svg>
  ),
};
