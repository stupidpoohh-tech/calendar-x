...
src/data/       Firestore 접근
  ...
  backup.ts       내보내기 · 가져오기 (파일 형식)
  restore.ts      복원 검증 · 교체 계획 · 실행 순서
...
**복원은 쓰고 나서 지운다. 순서가 곧 안전장치다.** (`data/restore.ts`)
검증 → 쓰기 → 결과 확인 → (교체일 때만) 지우기. 지우는 일은 언제나 맨 마지막이고,
쓰기가 한 건이라도 실패하면 거기서 멈춘다. 중간에 끊겨도 "지웠는데 복원이 안 된"
구간이 없다 — 기존 데이터와 복원 데이터가 둘 다 남고, 같은 파일로 다시 가져오면
이어서 끝난다. 여러 번 돌려도 같은 자리로 수렴한다.

`writeMany` · `deleteMany` 는 **실패를 던지지 않고 결과로 돌려준다.** 부르는 쪽이
`failed` 를 읽지 않으면 한 건도 저장되지 않아도 "가져왔습니다" 가 뜬다. 실제로 그랬다.

**가져오기 검증은 방어적으로 채우지 않는다.** `converters.ts` 가 빠진 값을 오늘 날짜나
기본값으로 메우는 것은 **이미 저장된 문서를 화면에 띄우기 위한** 것이다. 파일을 들여올
때 같은 일을 하면 사용자는 무엇이 달라졌는지 모른 채 원본과 다른 데이터를 갖게 된다.
`validateBackup()` 은 한 건이라도 걸리면 저장소를 건드리지 않고 이유와 함께 보고한다.

**"전체 교체" 는 네 컬렉션 모두에 적용된다.** `entries` 만 지우면 파일에 없는 잔고·대출·
고정 메모가 남아 교체가 아니게 된다.
